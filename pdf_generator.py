    ...
    def _draw_summary_page(self, c: canvas.Canvas, summary_data: List[Dict], 
        ...
        for entry in summary_data:
            ...
            if current_y < 80:  # 下部に余白を確保（品目ごとの総数セクション用）
                ...
                c.drawString(55 * mm, current_y - 12, "品目")
                c.drawString(95 * mm, current_y - 12, "フル箱")
                c.drawString(120 * mm, current_y - 12, "端数箱")
                c.drawString(145 * mm, current_y - 12, "TOTAL")
                c.drawString(170 * mm, current_y - 12, "総数")
                # ヘッダー下線
